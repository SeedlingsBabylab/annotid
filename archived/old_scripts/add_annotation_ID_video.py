...
import shutil
import sys
""" This script adds annotation ids to the usedID.txt file"""

# For now, usedIDs are loaded into a global variable (a set), then written
# out from there.
usedID = set()
...
# by reading through the usedID file. Need to come up with a better way to
# manage these.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Add annotation ids to video (opf) files')
    parser.add_argument(
        'opf_file',
        help='Full path the the opf file you want to add annotids to')
    parser.add_argument(
        'id_file',
        help='Full path to the usedID file that you would like to use!')
    ...
    # regex for timestamp (onset), so that we know there might be an
    # annotation on this line
    code_time = re.compile(r"\d{2}\:\d{2}\:\d{2}\:\d{3}")

    # Try to open the opf file as a zip file
    if not args.opf_file.endswith('.opf'):
        print(
            "Supplied file does not have an .opf extension! Are you sure it is an opf file?")
        sys.exit()

    # Making a temporary directory to extract the opf and then create it back
    # again!
    ...
    try:
        ...
        with open(os.path.join(tempdir, 'tmpfile'), 'w') as tmpfile:
            with open(os.path.join(tempdir, 'db')) as dbf:
                never = True
                # Iterate through each line of db file, modify if necessary,
                # and write it out!
                for line in dbf.readlines():
                    ...
                    if match:
                        ...
                        if not annotid:
                            ...
                            print(
                                'annotid was added to the line below. It looks like this:')
                            print(line)
                    tmpfile.write(line)
                if never:
                    print(
                        "No annotation IDs were added! Make sure this is what you expected!")
